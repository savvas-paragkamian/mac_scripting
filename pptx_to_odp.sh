...
LOG_FILE="$ROOT_DIR/pptx_to_odp_log.txt"
FAIL_FILE="$ROOT_DIR/pptx_to_odp_failed.txt"

: > "$LOG_FILE"
: > "$FAIL_FILE"

echo "Starting PPTX -> ODP conversion in: $ROOT_DIR" | tee -a "$LOG_FILE"

find "$ROOT_DIR" -type f -iname '*.pptx' -print0 |
while IFS= read -r -d '' file; do
  parent_dir="$(dirname "$file")"
  filename="$(basename "$file")"
  base_name="${filename%.*}"
  out_path="$parent_dir/$base_name.odp"
  ...
  if "$SOFFICE" --headless --convert-to odp --outdir "$parent_dir" "$file" >>"$LOG_FILE" 2>&1; then
    if [ -f "$out_path" ]; then
